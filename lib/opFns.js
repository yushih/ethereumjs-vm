...
module.exports = {
  ...
  EXTCODESIZE: function (address, runState, cb) {
    /*
      solc (v0.4.19) generates code to test if call target code size is zero
      and reverts if it is, so cheat it.
     */
    if (address.isZero()) {
        cb(null, new BN(1));
        return;
  ...
  CALL: function (gasLimit, toAddress, value, inOffset, inLength, outOffset, outLength, runState, done) {
    ...
    if (printer.printHook(toAddress, data)) {
        //Voodoo: we want to emulate 'return Print(0x0);' implementation
        //outLength sould be 0x20
        memStore(runState, outOffset, new Buffer(outLength), new BN(0), outLength)
    }
    var options = {
      gasLimit: gasLimit,
      value: value,
